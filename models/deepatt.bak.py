...
def layer_preprocess(x, params):
    if params.layer_preprocessor == "layer_norm":
        return layer_norm(x)
    else:
        return x


def layer_postprocess(x, y, params):
    if params.residual_dropout > 0.0:
        y = tf.nn.dropout(y, 1.0 - params.residual_dropout)

    x = x + y

    if params.layer_postprocessor == "layer_norm":
        return layer_norm(x)
    else:
        return x


def dynamic_rnn(cell, inputs, sequence_length, direction, time_major=False,
...
def rnn_layer(inputs, seq_len, hidden_size):
    with tf.variable_scope("recurrent"):
        x = inputs
        with tf.variable_scope("forward"):
            cell_fw = tf.nn.rnn_cell.BasicLSTMCell(hidden_size)
            y_fw = dynamic_rnn(cell_fw, x, seq_len, "forward")

        with tf.variable_scope("backward"):
            cell_bw = tf.nn.rnn_cell.BasicLSTMCell(hidden_size)
            y_bw = dynamic_rnn(cell_bw, x, seq_len, "backward")

        return y_fw + y_bw


def cnn_layer(inputs, mask, hidden_size, filter_width):
            ...
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("computation"):
                    if params.layer_type == "ffn_layer":
                        y = ffn_layer(
                            layer_preprocess(x, params),
                            params.filter_size,
                            params.hidden_size,
                            1.0 - params.relu_dropout,
                        )
                        x = layer_postprocess(x, y, params)
